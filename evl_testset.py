...
import atthis.datas
from atthis.metrics import MetricsTracker
from atthis.utils import tensors_to_device, RichProgressBarTheme, MyMetricsTextColumn, BatchesProcessedColumn
...
def main(config):
    ...
    model =  getattr(atthis.models, config["train_conf"]["audionet"]["audionet_name"]).from_pretrain(
        model_path,
        sample_rate=config["train_conf"]["datamodule"]["data_config"]["sample_rate"],
        **config["train_conf"]["audionet"]["audionet_config"],
    )
    video_model = getattr(atthis.videomodels, config["train_conf"]["videonet"]["videonet_name"])(
    ...
    datamodule: object = getattr(atthis.datas, config["train_conf"]["datamodule"]["data_name"])(
